    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                <h1 className="text-3xl font-bold text-gray-800">{viewMode === 'week' ? 'Weekly Schedule' : 'Monthly Calendar'}</h1>
                <div className="flex items-center space-x-2 flex-wrap gap-2">
{user?.role === Role.Librarian && (
                        <>
                            <Button onClick={() => { const defaultType = eventTypes.find(t => t.closesLibrary) || eventTypes.find(t => t.name.toLowerCase() === 'closure') || eventTypes[0]; setClosureForm({ title: '', date: weekDates[0] || new Date().toISOString().split('T')[0], endDate: weekDates[0] || new Date().toISOString().split('T')[0], allDay: true, typeId: defaultType?.id, periodStart: undefined, periodEnd: undefined, description: '' }); setClosureMode('create'); setClosureEditingId(null); setIsClosureModalOpen(true); }} variant="primary">Add Closure</Button>
