                    ...
                    {user?.role === Role.Librarian && (
                        <>
                            ...
                            <div className="relative inline-block">
                                <Button variant="secondary" onClick={() => setIsExportMenuOpen(!isExportMenuOpen)}>Import/Export</Button>
                                {isExportMenuOpen && (
                                    <div className="absolute mt-2 right-0 z-10 bg-white dark:bg-slate-800 dark:text-slate-100 border border-gray-200 dark:border-slate-700 rounded shadow p-3 space-y-3 w-72">
                                        ...
                                        <div className="flex gap-2">
                                            <Button variant="secondary" className="text-xs" onClick={async () => {
                                                ...
                                                if (fmt === '1') {
                                                    try {
                                                        const data = await api.exportShifts();
                                                    ...
                                                    } catch { alert('Export JSON failed'); }
                                                } else if (fmt === '2') {
                                                    try {
                                                        const data = await api.exportShifts();
                                                        const rows = data.shifts.map(s => ({ date: s.date, period: s.period, monitorIds: s.monitorIds.join(';') }));
                                                        const { downloadCsv } = await import('../../utils/csv');
                                                        downloadCsv('shifts.csv', rows, ['date', 'period', 'monitorIds']);
                                                    } catch { alert('Export CSV failed'); }
                                                }
                                            }}>Export</Button>
                                            <label className="inline-flex items-center">
                                                <input type="file" accept=".json,.csv,application/json,text/csv" className="hidden" onChange={async (e) => {
                                                    const input = e.currentTarget;
                                                    const f = input.files?.[0];
                                                    if (!f) return;
                                                    try {
                                                        const t = await f.text();
                                                        if (f.name.toLowerCase().endsWith('.json') || t.trim().startsWith('{')) {
                                                            const p = JSON.parse(t);
                                                            await api.importShifts(p.shifts || []);
                                                        } else {
                                                            const { parseCsv } = await import('../../utils/csv');
                                                            const rows = parseCsv(t);
                                                            const shifts = rows.map(r => ({ date: r.date, period: Number(r.period), monitorIds: (r.monitorIds || '').split(/[;,\s]+/).filter(Boolean) }));
                                                            await api.importShifts(shifts);
                                                        }
                                                        fetchShifts();
                                                        alert('Imported shifts successfully');
                                                    } catch { alert('Import failed'); } finally { input.value = ''; }
                                                }} />
                                                <Button variant="secondary" className="text-xs" onClick={(ev) => ((ev.currentTarget.previousElementSibling as HTMLInputElement) || (ev.currentTarget.parentElement?.querySelector('input[type=file]') as HTMLInputElement))?.click()}>Import</Button>
                                            </label>
                                        </div>
                                        ...
                                        <div className="flex gap-2">
                                            <Button variant="secondary" className="text-xs" onClick={async () => {
                                                ...
                                                if (fmt === '1') {
                                                    try {
                                                        const data = await api.exportPeriods();
                                                        const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
                                                        const url = URL.createObjectURL(blob);
                                                        const a = document.createElement('a');
                                                        a.href = url;
                                                    ...
                                                    try {
                                                        const d = await api.exportPeriods();
                                                        const { downloadCsv } = await import('../../utils/csv');
                                                        downloadCsv('periods.csv', d.definitions, ['period', 'duration', 'startTime', 'endTime']);
                                                    } catch { alert('Export CSV failed'); }
                                                }
                                            }}>Export</Button>
                                            <label className="inline-flex items-center">
                                                <input type="file" accept=".json,.csv,application/json,text/csv" className="hidden" onChange={async (e) => {
                                                    ...
                                                    try {
                                                        ...
                                                        if (f.name.toLowerCase().endsWith('.json') || t.trim().startsWith('{')) {
                                                            const p = JSON.parse(t);
                                                            await api.importPeriods(p.definitions || []);
                                                        } else {
                                                            const { parseCsv } = await import('../../utils/csv');
                                                            const rows = parseCsv(t);
                                                            const defs = rows.map(r => ({ period: Number(r.period), duration: Number(r.duration), startTime: r.startTime, endTime: r.endTime }));
                                                            await api.importPeriods(defs);
                                                        }
                                                        alert('Imported periods successfully');
                                                    } catch { alert('Import failed'); } finally { input.value = ''; }
                                                }} />
                                                <Button variant="secondary" className="text-xs" onClick={(ev) => ((ev.currentTarget.previousElementSibling as HTMLInputElement) || (ev.currentTarget.parentElement?.querySelector('input[type=file]') as HTMLInputElement))?.click()}>Import</Button>
                                            </label>
                                        ...
                                        <div className="flex gap-2">
                                            <Button variant="secondary" className="text-xs" onClick={async () => {
                                                ...
                                                if (fmt === '1') {
                                                    try {
                                                        const d = await api.exportEventTypes();
                                                        const blob = new Blob([JSON.stringify(d, null, 2)], { type: 'application/json' });
                                                        const url = URL.createObjectURL(blob);
                                                        const a = document.createElement('a');
                                                        a.href = url;
                                                        a.download = 'event_types.json';
                                                        a.click();
                                                        URL.revokeObjectURL(url);
                                                    } catch { alert('Export JSON failed'); }
                                                } else if (fmt === '2') {
                                                    try {
                                                        const d = await api.exportEventTypes();
                                                        const { downloadCsv } = await import('../../utils/csv');
                                                        downloadCsv('event_types.csv', d.types, ['name', 'color', 'icon', 'closesLibrary']);
                                                    } catch { alert('Export CSV failed'); }
                                                }
                                            }}>Export</Button>
                                            <label className="inline-flex items-center">
                                                <input type="file" accept=".json,.csv,application/json,text/csv" className="hidden" onChange={async (e) => {
                                                    ...
                                                    try {
                                                        ...
                                                        if (f.name.toLowerCase().endsWith('.json') || t.trim().startsWith('{')) {
                                                            const p = JSON.parse(t);
                                                            await api.importEventTypes(p.types || []);
                                                        } else {
                                                            const { parseCsv } = await import('../../utils/csv');
                                                            const rows = parseCsv(t);
                                                            const types = rows.map(r => ({ name: r.name, color: r.color, icon: r.icon || undefined, closesLibrary: String(r.closesLibrary || '').toLowerCase() === 'true' }));
                                                            await api.importEventTypes(types);
                                                        }
                                                        alert('Imported event types successfully');
                                                    } catch { alert('Import failed'); } finally { input.value = ''; }
                                                }} />
                                                <Button variant="secondary" className="text-xs" onClick={(ev) => ((ev.currentTarget.previousElementSibling as HTMLInputElement) || (ev.currentTarget.parentElement?.querySelector('input[type=file]') as HTMLInputElement))?.click()}>Import</Button>
                                            </label>
